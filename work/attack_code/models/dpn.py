...
class DPN(object):
    def __init__(self, layers=68):
        self.layers = layers
        self.globel_name = "dpn%d_"%(layers)
    ...
    def net(self, input, class_dim=1000):
        ...
        conv1_x_1 = fluid.layers.conv2d(
            ...
            bias_attr=False,
            name="conv1",
            param_attr=ParamAttr(name=self.globel_name+"conv1_weights"), )
        ...
        conv1_x_1 = fluid.layers.batch_norm(
            ...
            is_test=False,
            name="conv1_bn",
            param_attr=ParamAttr(name=self.globel_name+'conv1_bn_scale'),
            bias_attr=ParamAttr(self.globel_name+'conv1_bn_offset'),
            moving_mean_name=self.globel_name+'conv1_bn_mean',
            moving_variance_name=self.globel_name+'conv1_bn_variance', )
        ...
        for gc in range(4):
            ...
            convX_x_x = self.dual_path_factory(
                convX_x_x, R, R, bw, inc, G, _type1, name=self.globel_name + "dpn" + str(match))
            for i_ly in range(2, k_sec[gc] + 1):
                num += 1
                if num in match_list:
                    num += 1
                convX_x_x = self.dual_path_factory(
                    convX_x_x, R, R, bw, inc, G, _type2, name=self.globel_name + "dpn" + str(num))
        ...
        conv5_x_x = fluid.layers.batch_norm(
            ...
            param_attr=ParamAttr(name=self.globel_name+'final_concat_bn_scale'),
            bias_attr=ParamAttr(self.globel_name+'final_concat_bn_offset'),
            moving_mean_name=self.globel_name + 'final_concat_bn_mean',
            moving_variance_name=self.globel_name + 'final_concat_bn_variance', )
        ...
        fc6 = fluid.layers.fc(input=pool5,
                              size=class_dim,
                              param_attr=ParamAttr(initializer=fluid.initializer.Uniform(-stdv, stdv), name=self.globel_name + 'fc_weights'),
                              bias_attr=ParamAttr(name=self.globel_name + 'fc_offset'))
