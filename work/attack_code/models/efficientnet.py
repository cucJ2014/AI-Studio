...
class EfficientNet():
    ...
    def net(self, input, class_dim=1000, is_test=False):
        ...
        conv = self.conv_bn_layer(conv,
                                  ...
                                  name='',
                                  conv_name=self.global_name + '_conv_head',
                                  bn_name=self.global_name + '_bn1')
        ...
        param_attr, bias_attr = init_fc_layer(class_dim, self.global_name + '_fc')
        out = fluid.layers.fc(pool, class_dim, name=self.global_name + '_fc', param_attr=param_attr, bias_attr=bias_attr)
        return out
    ...
    def _conv_stem_norm(self, inputs, is_test):
        ...
        bn = self.conv_bn_layer(inputs, num_filters=out_channels, filter_size=3, stride=2, bn_act=None,
                                ...
                                bn_eps=self._bn_eps, name='', conv_name=self.global_name + '_conv_stem', bn_name=self.global_name + '_bn0')
    ...
    def extract_features(self, inputs, is_test):
        ...
        for block_args in self._blocks_args:
            ...
            conv = self.mb_conv_block(conv, block_args, is_test, drop_connect_rate, self.global_name + '_blocks.' + str(idx) + '.')
